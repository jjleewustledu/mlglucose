classdef (Sealed) C11GlucoseKit < handle & mlkinetics.TracerKit
    ...
    methods (Static)
        function this = instance(varargin)
            ...
            if (isempty(uniqueInstance))
                this = mlglucose.C11GlucoseKit(ipr.imaging, ipr.taus);
                uniqueInstance = this;
            else
                this = uniqueInstance;
            end
    ...
    methods (Access = private)
        function this = C11GlucoseKit(imaging, taus)
            re = regexp(imaging.fileprefix, '\S+_ses-(?<dtm>\d{14})_trc-(?<tra>\w+)_\S+', 'names');
            assert(contains(re.trc, 'glc', 'IgnoreCase', true));
